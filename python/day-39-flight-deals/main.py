...
for destination in sheet_data:
    ...
    flights = flight_search.check_flights(to_city=destination["iataCode"],
                                          from_time=TOMORROW, to_time=SIX_MONTHS)

    cheapest_flight = cheapest_flights(flights)
    print(f"The cheapest flight to {destination['city']} is £{cheapest_flight.price}")
    time.sleep(2)
